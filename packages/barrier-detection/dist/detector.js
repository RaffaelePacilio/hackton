...
export class BarrierDetectionEngine {
    detect(model, contract, options = {}) {
        ...
        for (const el of allElements) {
            ...
            if (this.hasUnmetPreference(el, contract)) {
                const severity = el.confidence < minConfidence ? "unknown" : "preference-mismatch";
                barriers.push(this.buildBarrier(el, available, severity));
            }
        }
        return barriers;
     ...
     */
    classifyCapabilitySeverity(el, missing, minConfidence) {
        ...
        if (el.confidence < minConfidence)
            return "unknown";
        ...
        if (missing.length === el.requiredCapabilities.length)
            return "blocking";
        ...
        if (missing.some((cap) => noSubstitute.includes(cap)))
            return "blocking";
     ...
     */
    hasUnmetPreference(el, contract) {
    ...
    buildBarrier(el, available, severity) {
        return {
