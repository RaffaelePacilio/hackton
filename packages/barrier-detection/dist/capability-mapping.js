 ...
 */
export function deriveAvailableCapabilities(contract) {
    const isUsable = (availability) => availability === "available";
    const available = [];
    if (isUsable(contract.input.keyboard))
        available.push("keyboard");
    if (isUsable(contract.input.pointer))
        available.push("pointer");
    if (isUsable(contract.input.touch))
        available.push("touch");
    if (isUsable(contract.input.voice))
        available.push("voice");
    if (isUsable(contract.input.switch))
        available.push("switch");
    if (isUsable(contract.actions.drag))
        available.push("drag");
    if (isUsable(contract.actions.precisionTargeting))
        available.push("precision-targeting");
    if (isUsable(contract.actions.complexShortcuts))
        available.push("complex-shortcuts");
